...
网站原本规划了 10 个真实素材位，包括 8 张 App 截图和 2 个教学素材。2026-09-19 已从 `appstore_materials` 目录根部的手动截屏中接入全部 App 素材位和 Apple Photos 教学图；`generated_en_6.9` 中的宣传版没有使用。页面优先加载由原图压缩出的 AVIF，并保留原尺寸 PNG 回退。SD 读卡器实拍素材位已按最终决定移除，目前没有可见素材占位。正式构建仍会在未来出现任何 `data-asset-placeholder` 时失败，避免误把占位内容发布上线。
